...
<div class="vst-board-wrap">
    ...
    <div class="vst-list-item <?php echo $is_notice ?>">
        ...
        <div class="vst-col-subject" style="padding-left:<?php echo $list[$i]['reply'] ? (strlen($list[$i]['wr_reply'])*10 + 15) : '15'; ?>px">
            <?php if ($is_category && $list[$i]['ca_name']) { ?>
                <a href="<?php echo $list[$i]['ca_name_href'] ?>" class="vst-badge vst-badge-cate"><?php echo $list[$i]['ca_name'] ?></a>
            <?php } ?>
            ...
            <a href="<?php echo $list[$i]['href'] ?>">
                ...
                <?php if (strstr($list[$i]['wr_option'], 'secret')) { ?>
                    <i class="fa fa-lock" class="vst-icon-lock"></i>
                <?php } ?>
                ...
                <?php if (isset($list[$i]['icon_new']) && $list[$i]['icon_new']) echo '<span class="vst-badge badge-new" class="badge-new-inline">N</span>'; ?>
                <?php if (isset($list[$i]['icon_hot']) && $list[$i]['icon_hot']) echo '<span class="vst-badge badge-hot" class="badge-hot-inline">H</span>'; ?>
                <?php if (isset($list[$i]['icon_file']) && $list[$i]['icon_file']) echo '<i class="fa fa-download" class="vst-icon-sm"></i>'; ?>
                <?php if (isset($list[$i]['icon_link']) && $list[$i]['icon_link']) echo '<i class="fa fa-link" class="vst-icon-sm"></i>'; ?>
            </a>
        </div>

        <div class="vst-info-row vst-info-row--desktop"> <div class="vst-col-writer"><?php echo $list[$i]['name'] ?></div>
            <div class="vst-col-date"><?php echo $list[$i]['datetime2'] ?></div>
            <div class="vst-col-hit"><i class="fa fa-eye" class="vst-icon-mr"></i><?php echo $list[$i]['wr_hit'] ?></div>
            <?php if ($is_good) { ?><div class="vst-col-good"><i class="fa fa-thumbs-up" class="vst-icon-mr"></i><?php echo $list[$i]['wr_good'] ?></div><?php } else { ?><div></div><?php } ?>
        </div>
    ...
    <?php if (count($list) == 0) { ?>
    <div class="vst-empty-box">
        <i class="fa fa-exclamation-circle" class="vst-empty-icon"></i><br>
        게시물이 없습니다.
    </div>
    <?php } ?>
